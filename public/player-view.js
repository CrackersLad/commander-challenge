import { db, functions } from './firebase-setup.js?v=4.22';
import { fetchDeckPriceLocal } from './deck-parser.js?v=4.22';
...
export function initPlayerViewModule(utils, state) {
    ...
    async function rollCommanders() {
        ...
        try {
            ...
            const { playBoosterPackReveal } = await import('./pack-animation.js?v=4.22');
    ...
    async function renderInteractiveDraft(activeDraft, container, s, players) {
        ...
        if (activeDraft.format === 'async_draft') { const { renderAsyncDraft } = await import('./draft-async.js?v=4.22'); renderAsyncDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'snake_draft') { const { renderSnakeDraft } = await import('./draft-snake.js?v=4.22'); renderSnakeDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'burn_draft') { const { renderBurnDraft } = await import('./draft-burn.js?v=4.22'); renderBurnDraft(activeDraft, container, s, state.currentPlayerId, players, utils); }
    }
    ...
    window.interactiveDraftAction = async (actionType, payload, event) => {
        ...
        if (actionType === 'async_pick') { const { handleAsyncPick } = await import('./draft-async.js?v=4.22'); await handleAsyncPick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'snake_pick') { const { handleSnakePick } = await import('./draft-snake.js?v=4.22'); await handleSnakePick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'burn_pick') { const { handleBurnPick } = await import('./draft-burn.js?v=4.22'); await handleBurnPick(payload, state.currentRoom, state.currentPlayerId, utils); }
    };
