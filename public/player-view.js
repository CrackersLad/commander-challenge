import { db, functions } from './firebase-setup.js?v=0.29';
import { fetchDeckPriceLocal } from './deck-parser.js?v=0.29';
...
export function initPlayerViewModule(utils, state) {
    ...
    async function renderInteractiveDraft(activeDraft, container, s, players) {
        ...
        if (activeDraft.format === 'async_draft') { const { renderAsyncDraft } = await import('./draft-async.js?v=0.29'); renderAsyncDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'snake_draft') { const { renderSnakeDraft } = await import('./draft-snake.js?v=0.29'); renderSnakeDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'burn_draft') { const { renderBurnDraft } = await import('./draft-burn.js?v=0.29'); renderBurnDraft(activeDraft, container, s, state.currentPlayerId, players, utils); }
    }
    ...
    window.interactiveDraftAction = async (actionType, payload, event) => {
        ...
        if (actionType === 'async_pick') { const { handleAsyncPick } = await import('./draft-async.js?v=0.29'); await handleAsyncPick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'snake_pick') { const { handleSnakePick } = await import('./draft-snake.js?v=0.29'); await handleSnakePick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'burn_pick') { const { handleBurnPick } = await import('./draft-burn.js?v=0.29'); await handleBurnPick(payload, state.currentRoom, state.currentPlayerId, utils); }
    };
