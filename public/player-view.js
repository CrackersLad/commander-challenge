import { db, functions } from './firebase-setup.js?v=4.26';
import { fetchDeckPriceLocal } from './deck-parser.js?v=4.26';
...
export function initPlayerViewModule(utils, state) {
    ...
    async function rollCommanders() {
        ...
        try {
            ...
            const { playBoosterPackReveal } = await import('./pack-animation.js?v=4.26');
    ...
    async function renderInteractiveDraft(activeDraft, container, s, players) {
        ...
        if (activeDraft.format === 'async_draft') { const { renderAsyncDraft } = await import('./draft-async.js?v=4.26'); renderAsyncDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'snake_draft') { const { renderSnakeDraft } = await import('./draft-snake.js?v=4.26'); renderSnakeDraft(activeDraft, container, s, state.currentPlayerId, players, utils); } 
        else if (activeDraft.format === 'burn_draft') { const { renderBurnDraft } = await import('./draft-burn.js?v=4.26'); renderBurnDraft(activeDraft, container, s, state.currentPlayerId, players, utils); }
    }
    ...
    function renderFinalForm(data) {
        ...
        document.getElementById('brewMoxfield').onclick = () => {
            playSound('sfx-click');
            const setPart = data.card_set ? ` (${data.card_set.toUpperCase()})` : '';
            const numPart = data.collector_number ? ` ${data.collector_number}` : '';
            const decklist = `1 ${data.selected}${setPart}${numPart} *CMDR*`;
            const url = `https://www.moxfield.com/import?c=${encodeURIComponent(decklist)}`;
            window.open(url, '_blank');
            showToast("Opening Moxfield importer with exact card art...", false, 3000, true);
        };
        ...
        document.getElementById('brewArchidekt').onclick = async () => {
            ...
            try {
                let scryfallId = data.scryfall_id;
                if (!scryfallId) {
                    ...
                    scryfallId = scryData.id;
                }
                
                const payload = [{ "c": "c", "f": 0, "q": 1, "u": scryfallId }];
                ...
                const setPart = data.card_set ? ` (${data.card_set.toUpperCase()})` : '';
                const numPart = data.collector_number ? ` ${data.collector_number}` : '';
                navigator.clipboard.writeText(`${data.selected}${setPart}${numPart}`).then(() => {
    ...
    window.interactiveDraftAction = async (actionType, payload, event) => {
        ...
        if (actionType === 'async_pick') { const { handleAsyncPick } = await import('./draft-async.js?v=4.26'); await handleAsyncPick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'snake_pick') { const { handleSnakePick } = await import('./draft-snake.js?v=4.26'); await handleSnakePick(payload, state.currentRoom, state.currentPlayerId, utils); } 
        else if (actionType === 'burn_pick') { const { handleBurnPick } = await import('./draft-burn.js?v=4.26'); await handleBurnPick(payload, state.currentRoom, state.currentPlayerId, utils); }
    };
